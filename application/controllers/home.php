...
class Home extends CI_Controller {
    ...
    public function listing($category, $city_id = null) {
        ...
        if ($category) {
            ...
            $data['trending_offers'] = $this->trendingOffersCoupons();
            $data['pagination']='';
            //print_r($data['trending_offers']);die;
    ...
    public function productDetails($id) {
        ...
        if ($id) {
            ...
            if (!empty($data['deal_details'])) {
                ...
                $data['categoryMegamenu'] = $this->category_model->getCategoryMegaMenu();
                if(!empty($data['deal_details'])){
                $data['couponsList'] = $this->category_model->getCouponsList($data['deal_details']->shopid);
                ...
                if ($data['deal_details']->item_type == 1 || $data['deal_details']->item_type == 2) {
                    $this->load->view('home/productDetails', $data);
                     ...
                     if(!empty($data['deal_details'])){
                         $data['dealsLists'] = $this->category_model->getdealsLists($data['deal_details']->shopid);
                     }
    ...
    public function onlineCoupons($catgId = NULL, $pagination = NULL) {
        ...
        $count_page = ceil($data['total_product'] / 10);
        if ($pagination == $count_page) {
            $rem = $data['total_product'] % 10;
            $start = $start - (10 - $rem);
        }
        ...
        //print_r($data['advertisement']);die;
        $data['trending_offers'] = $this->trendingOffersCoupons();
        
        $data['category_advertisements'][] = $this->deals_model->getAllAdvertisements($catgId);
        $this->load->view('home/onlineCoupons', $data);
    }

    /* Added function  for trending offers */

    public function trendingOffersCoupons($pagination = NULL) {
        ...
        $data['total_product_tre'] = $this->grocery_model->getallTrendingOffersCountRows();
        $array_total_product_tre = array($data['total_product_tre']);
        //print_r( $data['total_product']);die;
        $count_page = ceil($data['total_product_tre'] / 8);
        if ($pagination == $count_page) {
            $rem = $data['total_product_tre'] % 8;
            $start = $start - (8 - $rem);
        }
        return array_merge($array_total_product_tre, $data['trending_offers']);
    ...
    /* Ended function  for trending offers */
    ...
    public function setCookiesForSearch() {
        $country_value = $this->input->post('country_value');
...
//        echo $country_value.'-'.$location.'-'.$retailer_id;



        echo json_encode($cookieDetails);
        exit();
    }
